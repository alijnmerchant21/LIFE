...
contract perspection is Owned {
    
    uint public blockNumber = block.number;
    bytes32 public blockHashNow = blockhash(blockNumber);
    bytes32 public blockHashPrevious = blockhash(blockNumber - 1);
    ...
    //    return uint256[](transactionsArr);
   // }
    
    function getTransactionDetails(uint _id) view public returns (uint, string memory, uint, uint, address) {
    ...
    function getBuyerSellerDetails(uint _id) view public returns (uint, string memory, uint, string memory, uint, bytes32, bytes32) {
